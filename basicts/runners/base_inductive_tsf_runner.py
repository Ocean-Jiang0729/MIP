...
def get_mask(ratio, num_nodes):
#args.num_nodes = 20
    #ratio = 0.9
    mask_train = np.zeros(num_nodes, dtype=bool)
    
    num_train = int(num_nodes*ratio)
    num_val = int((num_nodes - num_train)/2)
    num_test = num_nodes - num_train - num_val
    
    mask_train[:num_train] = True
    np.random.shuffle(mask_train)
    ones = np.ones(num_nodes, dtype=bool)
    val_test_mask = ones^mask_train
    select = np.zeros(num_nodes-num_train, dtype=bool)
    select[:num_test]=True
    ...
    i=-1
    for j in range(len(val_test_mask)):
        if val_test_mask[j]!=0:
            i+=1
        if val_test_mask[j]!=0:
            if select[i]==1:
                #print("val", i,j, select[i], val_test_mask[j])
                mask_val[j]=True
            if select[i]==0:
                #print("test",i,j, select[i], val_test_mask[j])
                mask_test[j]=True

    return  mask_train,mask_test,mask_val

class InductiveTimeSeriesForecastingRunner(BaseRunner):
    """
    ...
    def __init__(self, cfg: Dict):
        ...
        self.mask_train,self.mask_test,self.mask_val = get_mask(self.train_ratio, self.num_nodes)
        print(self.mask_train,self.mask_test,self.mask_val)
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        if self.cl_param:
            ...
            forward_return["prediction"] = forward_return["prediction"][:, :cl_length, self.mask_train, :]
            forward_return["target"] = forward_return["target"][:, :cl_length, self.mask_train, :]
    ...
    def val_iters(self, iter_index: int, data: Union[torch.Tensor, Tuple]):
        ...
        # re-scale data
        forward_return = self.rescale_data(forward_return)
        forward_return["prediction"] = forward_return["prediction"][:, :, self.mask_val, :]
        forward_return["target"] = forward_return["target"][:, :, self.mask_val, :]
        # metrics
        for metric_name, metric_func in self.metrics.items():
            metric_item = self.metric_forward(metric_func, forward_return)
    ...
    def test(self):
        ...
        for _, data in enumerate(self.test_data_loader):
            ...
            prediction.append(forward_return["prediction"][:, :, self.mask_test, :])
            target.append(forward_return["target"][:, :, self.mask_test, :])
            inputs.append(forward_return["inputs"][:, :, self.mask_test, :])
