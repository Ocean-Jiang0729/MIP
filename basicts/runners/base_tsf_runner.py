...
from ..metrics import masked_mae_V, masked_mape_V, masked_rmse_V
...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: Dict):
        ...
        #print("FFFF if_rescale", self.if_rescale)
        # setup graph
        ...
        # define metric
        self.metrics = cfg.get("METRICS", {"MAE": masked_mae,"MAE_V":masked_mae_V, "RMSE": masked_rmse,"RMSE_V":masked_rmse_V, "MAPE": masked_mape, "MAPE_V": masked_mape_V, "WAPE": masked_wape, "MSE": masked_mse})
    ...
    def rescale_data(self, input_data: Dict) -> Dict:
        ...
        if self.if_rescale:
            ...
            if "out_v2" in input_data:
                if input_data["out_v2"] != None:
                    #input_data["out_v1"] = SCALER_REGISTRY.get(self.scaler["func"])(input_data["out_v1"], **self.scaler["args"])
                    input_data["out_v2"] = SCALER_REGISTRY.get(self.scaler["func"])(input_data["out_v2"], **self.scaler["args"])
    ...
    def evaluate(self, returns_all):
        ...
        for i in self.evaluation_horizons:
            ...
            for metric_name, metric_func in self.metrics.items():
                ...
                #print("pred", pred.shape, "real", real.shape)
