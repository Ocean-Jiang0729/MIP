...
class NodeFormerRunner(BaseTimeSeriesForecastingRunner):
    ...
    def forward(self, data: tuple, epoch:int = None, iter_num: int = None, train:bool = True, **kwargs) -> tuple:
        ...
        # feed forward
        model_return  = self.model(history_data=history_data, future_data=None, batch_seen=iter_num, epoch=epoch)
        ...
        if "target" not in model_return: model_return["target"] = self.select_target_features(history_data)
    ...
    def test(self):
        ...
        for _, data in enumerate(self.test_data_loader):
            ...
            # re-scale data
            prediction_rescaled = SCALER_REGISTRY.get(self.scaler["func"])(forward_return["prediction"], **self.scaler["args"])
            real_value_rescaled = SCALER_REGISTRY.get(self.scaler["func"])(forward_return["target"], **self.scaler["args"])
            # metrics
